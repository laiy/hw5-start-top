(function() {
  ...
  addEventHandlerToBubble = function() {
    ...
    return bubble.click(function() {
      if (bubble.attr('enabled') === 'true') {
        ...
        if (robot.state === 'working') {
          return console.log("大气泡： 楼主异步调用战斗力感人， 目测不超过" + countResult());
        }
      }
    });
  };
  ...
  robot = {
    ...
    getRandomOrder: function() {
      var button, count, i, j, len, original, random, ref;
      ref = this.order;
      for (i = j = 0, len = ref.length; j < len; i = ++j) {
        button = ref[i];
        this.order[i] = void 0;
      ...
      while (true) {
        ...
        if (this.order[random] === void 0) {
          this.order[random] = original[count++];
        }
        if (count === 5) {
          break;
        }
    ...
    printOrder: function() {
      return $('#info-bar').find('span').text(this.order);
    }
  };
